...
- **公開用データの書き出し** - 公開マークを付けたエントリだけを`public_entries.json`として書き出し、GitHub Pages等で公開できる
...
4. 書き出したファイルを`index.html`と同じ階層に配置し、リポジトリにコミット・push
...
### 公開ページに反映されない
- `public_entries.json`を書き出した後、実際にリポジトリへコミット・pushしたか確認してください
- `.gitignore`で`*.json`が除外対象になっていますが、`!public_entries.json`の例外設定が入っているか確認してください
